...
import com.musarayy.upipayment.ui.UpiBottomSheet
 ...
 */
class UpiPaymentActivity(activity: FragmentActivity){
    private var mUpiPaymentDetail: UpiPaymentDetail? = null
    ...
    companion object{
         ...
         */
        @JvmStatic
        val UPI_APPS: ArrayList<String> = arrayListOf("google pay","phonepe","paytm","bhim","mobikwik", "pay", "cred", "airtel", "dbs india", "amazon pay upi")
     ...
     */
    fun setPaymentDetail(upiPaymentDetail: UpiPaymentDetail): UpiPaymentActivity {
        mUpiPaymentDetail = upiPaymentDetail
        return this
    }
     ...
     */
    fun setUpiApps(data: ArrayList<String>): UpiPaymentActivity {
        mUpiApps.clear()
        if(data.isNotEmpty()){
            for(i in data){
     ...
     * callback listener
     */
    fun setCallBackListener(listener: OnUpiPaymentListener): UpiPaymentActivity {
    ...
    private fun startFragment() {
        if(!hasError()){
            val bundle = Bundle()
            bundle.putSerializable(UpiPaymentDetail.ARG_BUNDLE,mUpiPaymentDetail)
...
//        }
        if(mUpiPaymentDetail!!.txnRefId==""){
            mUpiPaymentDetail!!.txnRefId = generateString()
        }
        return false
    }
    ...
    class UpiPaymentFragment : Fragment() {
        ...
        override fun onActivityCreated(savedInstanceState: Bundle?) {
            ...
            val paymentDetail = arguments?.getSerializable(UpiPaymentDetail.ARG_BUNDLE) as UpiPaymentDetail
            ...
            if (paymentIntent.resolveActivity(requireContext().packageManager) != null)
        ...
        private fun getPaymentUri(upiPaymentDetail: UpiPaymentDetail): Uri {

            // Set Parameters for UPI
            val payUri = Uri.Builder()

            payUri.scheme("upi").authority("pay")
            payUri.appendQueryParameter("pa", upiPaymentDetail.vpa)
            payUri.appendQueryParameter("pn", upiPaymentDetail.name)
            payUri.appendQueryParameter("tid", upiPaymentDetail.txnId)
            payUri.appendQueryParameter("mc", upiPaymentDetail.payeeMerchantCode)
            payUri.appendQueryParameter("tr", upiPaymentDetail.txnRefId)
            payUri.appendQueryParameter("tn", upiPaymentDetail.description)
            payUri.appendQueryParameter("am", upiPaymentDetail.amount)
            payUri.appendQueryParameter("cu", upiPaymentDetail.currency)
        ...
        private fun startUpiBottomSheet(uri: Uri) {
            ...
            upiBottomSheet.show(requireFragmentManager(), UpiBottomSheet::class.java.simpleName)
        }
