package com.musarayy.upipayment.ui
...
import com.musarayy.upipayment.controller.UpiPaymentActivity
import com.musarayy.upipayment.adapter.UpiListAdapter
import com.wangsun.upi.payment.sample.R
import kotlinx.android.synthetic.main.upi_payment_upi_bottom_sheet.*
import java.util.*
...
class UpiBottomSheet : BottomSheetDialogFragment() {
    ...
    override fun getTheme(): Int = R.style.upi_style_Dialog_BottomSheet

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        mUri = Uri.parse(requireArguments().getString("uri"))
        mUpiApps = requireArguments().getStringArrayList(UpiPaymentActivity.ARG_UPI_APPS_LIST)!!
    }

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View
    {
        return inflater.inflate(R.layout.upi_payment_upi_bottom_sheet, container, false)
    }
    ...
    private fun getAllUpiApps() {
        ...
        if (mUpiApps.isNotEmpty()) {
            ...
            for (i in appList) {
                if (mUpiApps.contains(
                        i.loadLabel(requireContext().packageManager).toString().toLowerCase(Locale.getDefault())
                    )
                )
                    finalList.add(i)
            }
